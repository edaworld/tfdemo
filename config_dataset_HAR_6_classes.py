...
class Config(object):
    ...
    def __init__(self, X_train, X_test):
        ...
        self.gradient_noise_scale = None  # 人为的加入梯度噪声，有助于提高学习效果
...
def load_X(X_signals_paths):
    ...
    for signal_type_path in X_signals_paths:
        ...
        file.close()
    print("Before transpose shape is", np.array(X_signals).shape)
    ...
    for lambda_loss_amount in [0.005]:
        for clip_gradients in [15.0]:
            ...
            with open('{}_result_HAR_6.txt'.format(trial_name), 'a') as f:
                f.write(
                    str(learning_rate) + ' \t' + str(lambda_loss_amount) + ' \t' + str(clip_gradients) + ' \t' + str(
                        accuracy_out) + ' \t' + str(best_accuracy) + ' \t' + str(f1_score_out) + ' \t' + str(
                        best_f1_score) + '\n\n')
